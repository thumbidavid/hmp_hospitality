...
const form = useForm({
    ...
    max_event_capacity: '',
    website_url: '',
    featured_image: null, // Holds media ID uploaded by FileUploader
    is_featured: false,
    is_active: true,
    sort_order: 0,
    settings: [], // Selected setting IDs
    amenities: [], // Selected amenity IDs
    gallery: [],  // Array of { media_id: null, caption: '', is_cover: false, sort_order: 0 }
    key_experiences: [], // Array of { text: '', sort_order: 0 }
...
const addGalleryImage = () => {
    form.gallery.push({ media_id: null, caption: '', is_cover: false, sort_order: form.gallery.length })
}
...
const submit = () => {
    form.post(route('app.admin.properties.store'), {
        preserveScroll: true,
    })
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <PageBreadcrumb pageTitle="Create Property" />
                    ...
                    <ul v-else class="space-y-6">
                        <li v-for="(img, index) in form.gallery" :key="index"
                            ...
                            <div class="w-80 min-w-[240px]">
                                ...
                                <FileUploader :id="`gal_file_${index}`" v-model="img.media_id" class="mt-1" />
                            </div>
