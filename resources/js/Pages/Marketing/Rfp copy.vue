...
const f = reactive({
    ...
    requirementType: "", programmeName: "", preferredDestination: "", flexibleDestination: false, arrivalDate: "", departureDate: "", flexibleDates: false,
...
// Hydrate from query parameters or Pinia store on mount
onMounted(() => {
    const params = new URLSearchParams(window.location.search)
    const pre = params.get("prop")

    // If arriving from a specific property detail, pre-populate that item in the shortlist
    if (pre) {
...
// Compute chosen properties dynamically from Pinia store, matched with real details from DB
const chosenProps = computed(() => {
    return page.props.properties.filter(p => shortlistStore.has(p.slug))
})

// Toggle shortlisted items directly on the form
...
// Retrieve success response from controller flash sessions
const done = computed(() => page.props.flash?.success || null)

const submit = () => {
    submitting.value = true

    // 1. Convert shortlisted property slugs to database integer IDs for sync()
    const propertyIds = chosenProps.value.map(p => p.id)

    // 2. Convert selected agency support service strings to database integer IDs
    ...
    // 3. Match buyer country name to country ID
    ...
    // 4. Match buyer type name to buyer type ID
    ...
    // 5. Convert requirementType to validation-compliant snake_case string
    ...
    // 6. Clever "Can't Decide" Mechanism
    ...
    // 7. Assemble backend validation-compliant payload matching StoreRfpSubmissionRequest
    const payload = {
        ...
        preferred_communication_method: f.commMethod.toLowerCase().replace(' ', '_'), // "Video Call" -> "video_call"
    ...
    router.post('/rfp', payload, {
        ...
        preserveState: true,
        onSuccess: () => {
            // Step 4: Clear shortlist Pinia store on successful submission
...
<template>
    ...
    <div v-if="done" class="pt-32 container-wide py-20">
        <div class="max-w-2xl mx-auto rounded-3xl border border-border bg-card p-10 lg:p-14 text-center">
            ...
            <h1 class="mt-6 font-heading text-3xl sm:text-4xl">Your RFP has been received.</h1>
            <p class="mt-4 text-muted-foreground">
                Thank you, {{ f.plannerName.split(" ")[0] || "buyer" }}. HMP Hospitality will review your brief, match
                it to represented members and respond within one business day.
            </p>
            <div
            ...
            <p v-if="done.note" class="mt-5 text-sm text-muted-foreground">{{ done.note }}</p>
            <p v-if="wantsAgency && f.agencyConsent" class="mt-5 text-sm text-muted-foreground">
            ...
            <div class="mt-9 flex flex-wrap gap-4 justify-center">
                <Link href="/portfolio" class="btn-primary">Back to portfolio</Link>
                <Link href="/" class="btn-ghost">Return home</Link>
            </div>
        </div>
    </div>

    <!-- Active Form Page Layout -->
    <div v-else>
        <!-- Hero Section -->
        ...
        <!-- Top Shortlist Banner Selector (Change shortlist on the form) -->
        ...
        <!-- Main Form Content & Sidebar Grid -->
        <section class="container-wide grid gap-10 lg:grid-cols-[1.6fr_1fr] py-10">
            ...
            <!-- Sidebar list preview (Change shortlist directly inside the form) -->
            <aside class="lg:sticky lg:top-28 self-start space-y-5">
                <div class="rounded-3xl border border-border bg-card p-6 shadow-sm">
                    ...
                    <!-- Fallback: Can't Decide recommendations triggers -->
                    ...
                    <ul v-else class="mt-4 space-y-3">
                        <li v-for="p in chosenProps" :key="p.id" class="flex items-center justify-between gap-3">
                            ...
                            <!-- Cross button to instantly remove item on the form -->
                            <button @click="toggleProp(p.slug)"
